...
test('subscription relay preserves effort, auth, SSE, large contexts and fails closed without fallback', async () => {
  const requests: {url: string; body: any; key: unknown; beta: unknown}[] = [];
  ...
  const upstream = http.createServer(async (req,res) => {
    ...
    requests.push({url:req.url!, body:JSON.parse(Buffer.concat(chunks).toString()),key:req.headers['x-api-key'],beta:req.headers['anthropic-beta']});
  ...
  let configuredModels = [{id:'claude-opus-5',label:'Opus',contextWindow:1000000,maxEffort:'max'},{id:'claude-fable-5-1',label:'Fable',contextWindow:1000000,maxEffort:'max'}];
  const app = createApp({settings:{load:async()=>({proxyUrl:url,claudeModels:configuredModels})} as SettingsStore, profiles, mgmt, webDist:null});
  ...
  try {
    ...
    const modelList = await list.clone().json() as any;
    assert(modelList.data.every((m:any) => m.supports_1m === true && m.max_input_tokens === 1000000));
    ...
    assert.match(String(requests[0]!.beta), /context-1m-2025-08-07/);
    ...
    configuredModels = [configuredModels[1]!];
    res=await call(body);assert.equal(res.status,400);await res.text();
    list=await fetch(relayUrl+'/v1/models',{headers:{'x-api-key':'client-key'}});
    assert.deepEqual((await list.json() as any).data.map((m:any)=>m.id),['claude-fable-5-1']);
