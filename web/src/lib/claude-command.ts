...
export function claudeCommand(profile: string, model: string, effort: string, consoleUrl: string, configDir?: string, roles: {backgroundModel?:string;subagentModel?:string} = {}): string {
  ...
  for (const role of [roles.backgroundModel,roles.subagentModel]) if(role && !/^claude-[a-zA-Z0-9.-]+$/.test(role)) throw new Error('Invalid role model ID');
  ...
  return [
    ...
    ...['ANTHROPIC_MODEL', 'ANTHROPIC_DEFAULT_OPUS_MODEL', 'ANTHROPIC_DEFAULT_FABLE_MODEL', 'ANTHROPIC_DEFAULT_SONNET_MODEL', 'ANTHROPIC_DEFAULT_HAIKU_MODEL', 'CLAUDE_CODE_SUBAGENT_MODEL'].map(key => `${key}='${key === 'ANTHROPIC_DEFAULT_HAIKU_MODEL' && roles.backgroundModel ? `${roles.backgroundModel}[1m]` : key === 'CLAUDE_CODE_SUBAGENT_MODEL' && roles.subagentModel ? `${roles.subagentModel}[1m]` : selected}' \\`),
